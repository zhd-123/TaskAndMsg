...
DownloadConfig *DownloadConfig::instance()
{
    static DownloadConfig instance;
    return &instance;
}

void DownloadConfig::setConfigFile(const QString &filePath)
{
    if (filePath.isEmpty()) {
        return;
    }
    if (!filePath.contains("/")) {
        mConfigFilePath = QDir::fromNativeSeparators(filePath);
    } else {
        mConfigFilePath = filePath;
    }
}

ConfigParam DownloadConfig::readDownloadConfig()
{
    ...
    mConfigParam.url = settings.value("url").toString();
    mConfigParam.crc = settings.value("crc").toULongLong();
    mConfigParam.totalSize = settings.value("totalSize").toInt();
    mConfigParam.downloadSize = settings.value("downloadSize").toInt();
    mConfigParam.segmentCount = settings.value("segmentCount").toInt();
    mConfigParam.segmentArray.clear();
    for (int i = 0; i < mConfigParam.segmentCount; ++i) {
        SegmentParam param;
        param.index = settings.value("segment" + QString::number(i) + "/index").toInt();
        param.startPos = settings.value("segment" + QString::number(i) + "/startPos").toInt();
        param.endPos = settings.value("segment" + QString::number(i) + "/endPos").toInt();
        mConfigParam.segmentArray.append(param);
    }
    return mConfigParam;
}

void DownloadConfig::writeDownloadConfig(const ConfigParam &configParam)
{
    if (mConfigFilePath.isEmpty()) {
        return;
    }
    QString filePath = mConfigFilePath;
    ...
    QSettings settings(filePath, QSettings::IniFormat);
    settings.setValue("url", configParam.url);
    settings.setValue("crc", configParam.crc);
    settings.setValue("totalSize", configParam.totalSize);
    settings.setValue("downloadSize", configParam.downloadSize);
    settings.setValue("segmentCount", configParam.segmentCount);
    for (int i = 0; i < configParam.segmentCount; ++i) {
        settings.setValue("segment" + QString::number(i) + "/index", configParam.segmentArray[i].index);
        settings.setValue("segment" + QString::number(i) + "/startPos", configParam.segmentArray[i].startPos);
        settings.setValue("segment" + QString::number(i) + "/endPos", configParam.segmentArray[i].endPos);
    }
    settings.sync();
}
...
quint32 getCrc32(const QByteArray &data) {
